    ...
    <div class="bg-white dark:bg-gray-800 h-[97vh] overflow-auto shadow-sm md:rounded-3xl">
        ...
        <div class="sticky flex top-0 h-[3rem] border-b bg-white">
            <x-breadcrumb>
                ...
                <x-breadcrumb.link :href="route('roles.create')" :text="__('label.edit')" />
            </x-breadcrumb>
        </div>
                    ...
                    <div class="grid grid-cols-2 gap-5 px-6 pb-6">
                        @foreach ($permissionGroups as $group => $permissions)
                            <div x-data="{
                                allChecked: false,
                                toggleAll() {
                                    let allSelect = this.$refs.allSelect.checked;
                                    let checkboxes = [...this.$refs.checkboxes.querySelectorAll('.child-checkbox')];
                                    checkboxes.map(el => el.checked = allSelect);
                                },
                                updateSelectAll() {
                                    let checkboxes = [...this.$refs.checkboxes.querySelectorAll('.child-checkbox')];
                                    this.allChecked = checkboxes.length > 0 && checkboxes.every(el => el.checked);
                                }
                            }" x-init="updateSelectAll()" class="bg-slate-50 rounded-lg border">
                                <div class="flex justify-between items-center p-3 border-b">
                                    <div class="text-lg text-dark">
                                        {{ ucfirst($group) }}
                                    </div>
                                    <div class="flex items-center">
                                        <input id="{{ $group }}" x-ref="allSelect" type="checkbox" x-model="allChecked" @click="toggleAll()" value=""
                                ...
                                <div x-ref="checkboxes" class="grid grid-cols-2 gap-3 p-3">
                                    @foreach ($permissions as $permission)
                                        <div class="flex items-center">
                                            <input id="{{ $permission->name }}" type="checkbox" {{ in_array($permission->name, $selectedPermissions) ? 'checked' : '' }}
                                                name="permissions[]" @change="updateSelectAll()"
